(ns freee.main
  "Kotodama WASM entrypoint for the Freee clean-room actor (L4) — Clojure port.
  ...
  facts (`freee.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Deal"        :plural "deals"        :id-prefix "freee_dea"
    :fields [:companyId :issueDate :dueDate :amount :dueAmount :type :partnerId :partnerCode :refNumber :status]
    :required [:issueDate :dueDate]
    :coerce {:companyId :int :amount :int :dueAmount :int :partnerId :int}
    :refs {:companyId "Company" :partnerId "Partner"}}
   {:entity "AccountItem" :plural "accountitems" :id-prefix "freee_acc"
    :fields [:name :companyId :taxCode :accountCategory :available :walletableId :searchable]
    :required [:name :taxCode]
    :coerce {:companyId :int :taxCode :int :available :bool :walletableId :int :searchable :int}
    :refs {:companyId "Company" :walletableId "Walletable"}}
   {:entity "Partner"     :plural "partners"     :id-prefix "freee_par"
    :fields [:name :code :companyId :available :orgCode :transferFeeHandlingSide]
    :required [:name :code]
    :coerce {:companyId :int :available :bool :orgCode :int}
    :refs {:companyId "Company"}}
   {:entity "Invoice"     :plural "invoices"     :id-prefix "freee_inv"
    :fields [:invoiceNumber :issueDate :totalAmount :invoiceStatus :paymentType]
    :required [:invoiceNumber :issueDate]
    :coerce {:totalAmount :int}
    :refs {}}
   {:entity "Company"     :plural "companies"    :id-prefix "freee_com"
    :fields [:displayName :role :orgCode]
    :required [:displayName :role]
    :coerce {:orgCode :int}
    :refs {}}
   {:entity "Walletable"  :plural "walletables"  :id-prefix "freee_wal"
    :fields [:name :bankId :type :syncStatus]
    :required [:name :type]
    :coerce {:bankId :int}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"freee.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "freee-compat" :tier tier :entities entities} 200])
